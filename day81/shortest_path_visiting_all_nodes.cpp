...
int shortestPathLength(vector<vector<int>> &graph)
{
    ...
    for (int i = 0; i < n; i++)
    { // distance to neighbor is 1 , else infi
        for (int j : graph[i])
        {
            W[i][j] = 1;
        }
    ...
    for (int i = 0; i < n; i++)
    { // nearest dist b/w each pair of nodes
        for (int j = 0; j < n; j++)
        {
            for (int k = 0; k < n; k++)
            {
    ...
    for (int mask = 0; mask < 1 << n; mask++)
    {
        ...
        for (int j : nz)
        { // j is the last node visited by mask
            for (int k : nz)
            { // k is the node last visited by the neighbor mask
                if (j != k)
                {
